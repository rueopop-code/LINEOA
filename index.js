...
function buildStatusUpdateFlex(order, status) {
  const labels = {
    ...
    confirmed: { emoji:'☑️', text:'ยืนยันแล้ว',    desc:'ร้านยืนยันออเดอร์เรียบร้อย กำลังจัดเตรียม', color:'#27AE60' },
    ...
    cancelled: { emoji:'✖️', text:'ยกเลิก',         desc:'ออเดอร์ถูกยกเลิก ติดต่อร้านหากมีข้อสงสัย', color:'#7F8C8D' },
  ...
  if (discount > 0) {
    totalContents.push({
      type:'box', layout:'horizontal', margin:'md',
      contents:[
        { type:'text', text:'ส่วนลด', size:'sm', color:'#e8593c', flex:1 },
  ...
  totalContents.push({
    type:'box', layout:'horizontal', margin:'sm',
    contents:[
      { type:'text', text:'รวม', size:'md', color:'#333', weight:'bold', flex:1 },
      { type:'text', text:`฿${total.toLocaleString()}`, size:'lg', color:'#C0392B', weight:'bold', align:'end' }
    ]
  });
  ...
  return {
    type:'flex',
    altText: `${lbl.text} — ออเดอร์ #${order.order_id} ฿${total.toLocaleString()}`,
    contents: {
      type:'bubble',
      header: {
        type:'box', layout:'vertical',
        ...
        contents: [
          { type:'text', text: lbl.text, color:'#ffffff', size:'xl', weight:'bold' },
          { type:'text', text:`#${order.order_id}`, color:'#ffffff', size:'sm', margin:'xs', opacity:0.85 }
        ]
      },
      body: {
        ...
        contents: [
          ...
          { type:'separator', margin:'lg' },
          ...totalContents,
          { type:'text', text: date, size:'xs', color:'#aaa', margin:'md' },
          { type:'separator', margin:'sm' },
          { type:'text', text: lbl.desc, size:'sm', color:'#666', wrap:true, margin:'md', align:'center' }
        ]
      },
      footer: {
